...
const LogoutButton = () => {
  ...
  const handleClick = () => {
    if (state === 'default' || state === 'hover') {
      ...
      setTimeout(() => {
        setButtonClass('clicked door-slammed');
        updateButtonState('walking2');
        setTimeout(() => {
          setButtonClass('clicked door-slammed falling');
  ...
  return (
    <div onClick={()=>{handleLogOut()}} className={`logout_background ${state === 'default' ? 'background--light' : ''}`}>
      <button
        className={`logoutButton logoutButton--${state === 'default' ? 'dark' : 'light'} ${buttonClass}`}
        style={logoutButtonStates[state]}
